...
const ModalButton: React.FC<ModalButtonProps> = ({ onPress, icon, children, style, disabled, iconRight, textStyle }) => {
  ...
  return (
    <TouchableOpacity
      ...
      disabled={disabled}
    >
      {!iconRight && icon && <Feather name={icon} size={18} color={tokens?.colors?.buttonText ?? "#000000"} style={{ marginRight: 8 }} />}
      <Text style={{
      ...
      {iconRight && icon && <Feather name={icon} size={18} color={tokens?.colors?.buttonText ?? "#000000"} style={{ marginLeft: 8 }} />}
    </TouchableOpacity>
  );
};
...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ visible, onClose }) => {
  ...
  const renderIcon = (iconType: string) => {
    const color = tokens?.colors?.accent ?? "#000000";
    switch (iconType) {
      case 'brain':
        return <Brain size={iconSize} color={color} strokeWidth={2.5} />;
      case 'mic':
        return <Mic size={iconSize} color={color} strokeWidth={2.5} />;
      case 'siri':
        return <Ionicons name="phone-portrait-outline" size={iconSize} color={color} />;
      case 'crown':
        return <MaterialCommunityIcons name="crown-outline" size={iconSize} color={color} />;
      case 'search':
        return <Search size={iconSize} color={color} strokeWidth={2.5} />;
      case 'premium':
        return <MaterialCommunityIcons name="crown-outline" size={iconSize} color={color} />;
      default:
        return <Brain size={iconSize} color={color} strokeWidth={2.5} />;
    ...
    >
      <View style={{
        ...
        <View style={{
          ...
          {/* Icon */}
          <View>{renderIcon(steps[currentStep].iconType)}</View>
          {/* Body Text */}
          <Text style={{
            ...typography.body,
