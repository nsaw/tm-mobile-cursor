...
function fileSummary(filename) {
  ...
  CLICKABLES.forEach(Clickable => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      // Find handlers and accessibility props in prop string
      ...
      // Look for accessibility props in handler (illegal)
...
files.forEach(file => {
  ...
  summary.clickableBlocks.forEach((block, idx) => {
    ...
    if (block.illegalAccessInHandler) {
      console.log(`❌ [${file}] <${block.element} #${idx+1}>: Accessibility props must NOT be set inside handler!`);
      hasIssue = true;
    }
  });
