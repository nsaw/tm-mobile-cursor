...
function processFile(filePath) {
  ...
  const importRegex = /import\s*{\s*[^}]*designTokens[^}]*}\s*from\s*['"][^'"]*['"];?\s*\n?/g;
  ...
  if (designTokensRegex.test(content)) {
    ...
    const isInComponent = /export\s+(const|function)\s+\w+\s*[:=]\s*(React\.)?FC/.test(content);
    
    if (isInComponent) {
      // Add useTheme import if not present
      if (!content.includes('useTheme')) {
        const importMatch = content.match(/import\s+.*from\s+['"][^'"]*ThemeProvider['"]/);
        if (importMatch) {
          content = content.replace(importMatch[0], importMatch[0].replace('}', ', useTheme }'));
        } else {
          // Add new import
      ...
      if (!content.includes('const { tokens } = useTheme()')) {
        ...
        if (componentMatch) {
          ...
          const componentStart = content.indexOf(`export ${componentMatch[1]} ${componentName}`);
      ...
      if (!content.includes('function getStyles(tokens)')) {
        ...
        if (styleSheetMatch) {
          const styleSheetStart = content.indexOf('const styles = StyleSheet.create(');
          const styleSheetEnd = content.indexOf('});', styleSheetStart) + 3;
          const styleSheetContent = content.slice(styleSheetStart, styleSheetEnd);
