    ...
    def __init__(self, config_path: str = None):
        if config_path and os.path.exists(config_path):
            with open(config_path, 'r') as f:
                config = json.load(f)
                self.project_dirs = config.get('project_directories', [])
                self.ignore_patterns = config.get('ignore_patterns', [])
                self.linter_settings = config.get('linter_settings', {})
                self.monitoring = config.get('monitoring', {})
                self.fixing_strategies = config.get('fixing_strategies', {})
        else:
...
def main():
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), 'autolinter-python-config.json'
    )

    # Create autolinter instance
    autolinter = AutoLinter(config_path)
    ...
    for project_dir in autolinter.project_dirs:
