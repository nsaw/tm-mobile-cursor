...
const VALID_TEXT_ROLES = [
  'heading', 'body', 'caption', 'label', 'button', 'link',
  'title', 'subtitle', 'description', 'placeholder', 'error'
];

const VALID_VIEW_ROLES = [
  'container', 'card', 'button', 'input', 'list', 'item',
  'header', 'footer', 'section', 'navigation', 'modal',
  'overlay', 'tooltip', 'badge', 'divider', 'spacer'
];
...
function isValidRole(role: string, componentType: string): boolean {
  ...
  } else if (componentType === 'View' || componentType.startsWith('Touchable') || componentType === 'Pressable') {
...
function suggestRole(componentType: string, context: string): string {
  ...
  if (componentType === 'Text') {
    if (lowerContext.includes('title') || lowerContext.includes('heading')) return 'heading';
    if (lowerContext.includes('button') || lowerContext.includes('press')) return 'button';
    if (lowerContext.includes('link') || lowerContext.includes('url')) return 'link';
    if (lowerContext.includes('error') || lowerContext.includes('invalid')) return 'error';
    if (lowerContext.includes('caption') || lowerContext.includes('small')) return 'caption';
    if (lowerContext.includes('label')) return 'label';
    return 'body';
  } else {
    if (lowerContext.includes('button') || lowerContext.includes('press')) return 'button';
    if (lowerContext.includes('card') || lowerContext.includes('elevated')) return 'card';
    if (lowerContext.includes('container') || lowerContext.includes('wrapper')) return 'container';
    if (lowerContext.includes('list') || lowerContext.includes('item')) return 'list';
    if (lowerContext.includes('header')) return 'header';
    if (lowerContext.includes('footer')) return 'footer';
    if (lowerContext.includes('modal') || lowerContext.includes('overlay')) return 'modal';
    return 'container';
  }
}
          ...
          if (roleValue) {
            ...
            if (!isValidRole(roleValue, componentType)) {
              ...
              auditResults.issues.push({
                ...
                suggestion: `Consider using one of: ${componentType === 'Text' ? VALID_TEXT_ROLES.join(', ') : VALID_VIEW_ROLES.join(', ')}`
              });
            }
          } else {
