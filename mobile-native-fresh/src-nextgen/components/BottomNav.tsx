...
interface BottomNavProps {
  ...
  currentRoute?: string;
  onCreateNew?: () => void;
  layoutRole?: 'navigation' | 'navigation-primary' | 'navigation-secondary';
}
...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  const renderIcon = (item: any, isActive: boolean, isHome: boolean, isVoice: boolean) => { // eslint-disable-line @typescript-eslint/no-explicit-any
    const iconColor = isHome
      ...
      : designTokens.colors.text.muted; // Light gray for inactive
  ...
  return (
    <View 
      ...
      accessibilityLabel={`${layoutRole}: Bottom navigation`}
    >
      <View style={{
        ...
        <View style={{
          ...
          {navItems.map((item, index) => {
            const { icon, label, path: itemPath, action, iconType } = item;
            const isActive = currentRoute === itemPath;
            const isVoice = action === 'voice';
            const isHome = itemPath === '/';

            return (
              <TouchableOpacity
                key={itemPath || action || label}
                style={{
                  alignItems: 'center',
                ...
                accessible={true}
                accessibilityLabel={`${layoutRole}-item: ${label}`}
              >
        ...
        <View style={{
          ...
          <View style={{
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
              accessibilityLabel={`${layoutRole}-create-button: Create new thoughtmark`}
            >
