...
import performanceMonitorInstance from '../utils/PerformanceMonitor';
...
export const useEnvironment = () => {
  ...
  const validationSystem = useRef(ValidationSystem.getInstance());
  const performanceMonitor = useRef(performanceMonitorInstance);
  ...
  const validateEnvironment = useCallback(async (environment: Environment): Promise<{ isValid: boolean; errors: string[] }> => {
    try {
      ...
      const validationResult = await validationSystem.current.validateEnvironment();
  ...
  const toggleEnvironment = useCallback(async (): Promise<EnvironmentToggleResult> => {
    ...
    try {
      ...
      const _togglePromise = new Promise<EnvironmentToggleResult>((resolve, reject) => {
