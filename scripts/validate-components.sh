...
# Validate TypeScript compilation for components
log_info "Validating TypeScript compilation for components..."

# Create a temporary tsconfig for component validation
temp_tsconfig="/tmp/component-validation-tsconfig.json"
...
# Run TypeScript check on components only
if npx tsc --project "$temp_tsconfig" --noEmit 2>/dev/null; then
    log_success "TypeScript compilation for components successful"
else
    log_warning "TypeScript compilation for components has warnings (expected due to corruption)"
fi
