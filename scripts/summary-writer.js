...
class SummaryWriter {
    ...
    ensureDirectories() {
        if (!fs.existsSync(this.writeDir)) {
            ...
            console.log(`📁 Created summary write directory: ${this.writeDir}`);
        }
        
        if (!fs.existsSync(this.archiveDir)) {
            fs.mkdirSync(this.archiveDir, { recursive: true });
            console.log(`📁 Created summary archive directory: ${this.archiveDir}`);
        }
    }
    ...
    writeSummary(filename, content, metadata = {}) {
        try {
            ...
            console.log(`📄 Summary written: ${filename}`);
            ...
            console.error(`❌ Error writing summary ${filename}:`, error.message);
            return {
                success: false,
                error: error.message
            };
...
## Results
${executionResult.success ? '✅ Patch executed successfully' : '❌ Patch execution failed'}
...
`;
    ...
    archiveOldSummaries() {
        try {
            ...
            if (archivedCount > 0) {
                console.log(`📦 Archived ${archivedCount} old summaries`);
            }

            return archivedCount;
        } catch (error) {
            console.error('❌ Error archiving summaries:', error.message);
            return 0;
        }
    }
...
`;
            ...
            console.log('📋 Generated summaries INDEX.md');
